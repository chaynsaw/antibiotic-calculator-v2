...
export async function getWasteItems(drug: string, dose: string | number, method: string, form: string): Promise<WasteItem[]> {
  try {
    ...
    const matchingRows = rows.filter(row => {
      ...
      const formMatch = row[5]?.trim() === form;
      // For fixed doses, match exactly
      if (typeof dose === 'string') {
        return drugMatch && row[1]?.trim() === dose && methodMatch && formMatch;
      }
      ...
      return drugMatch && dose >= minDose && dose <= maxDose && methodMatch && formMatch;
    });
    
    if (!matchingRows.length) {
      throw new Error(`Data not found for ${drug} ${dose}mg with method: ${method} and form: ${form}`);
    }
